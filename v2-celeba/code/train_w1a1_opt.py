...
def train_w1a1_optimized():
    logger.info("=== Starting Optimized W1A1 Native Training (AdamW, LR=2e-4) ===")
    ...
    # 2. Train W1A1 Native with AdamW and 2e-4 LR
    w1a1_model = ResUNet_W1A1().to(DEVICE)
    w1a1_opt = torch.optim.AdamW(w1a1_model.parameters(), lr=2e-4, weight_decay=1e-2)
    ...
    logger.info(f"Optimized W1A1 Native checkpoint saved to {ckpt_path}")

if __name__ == "__main__":
    train_w1a1_optimized()
